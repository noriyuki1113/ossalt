...
const GUIDES: Record<string, GuideData> = {
  "notion-alternatives": {
    title: "Notion代替OSSを選ぶときのポイント",
    metaTitle: "Notion代替OSSを選ぶときのポイント | OSSアルタナティブ",
    metaDescription: "Notionの代わりに使えるOSSツールを選ぶ際の比較観点・注意点を解説。コスト・データ主権・機能面から最適なOSSを見つけるガイド。",
    intro: "Notionは多機能なオールインワンワークスペースですが、チーム規模の拡大とともに月額コストが膨らみやすく、データの外部保管に懸念を持つ企業も増えています。このガイドでは、Notion代替OSSを選ぶときに見るべきポイントを整理します。",
    audience: "Notionの月額費用を削減したい方、データを自社管理したい方、カスタマイズ性を重視する方",
    sections: [
      {
        heading: "比較時に見るべき観点",
        content: [
          "ドキュメント・Wiki機能の充実度: Notionの強みはドキュメント、データベース、Wikiが一体化していること。代替OSSがどこまでカバーしているか確認しましょう。",
          "リアルタイム共同編集: チームでの同時編集が必要なら、WebSocket対応やCRDTベースの共同編集機能があるかチェック。",
          "セルフホストの難易度: Docker Composeでサクッと立ち上がるか、Kubernetesが必要かで運用負荷が大きく変わります。",
          "データのエクスポート性: Notionからの移行時にMarkdownやCSVでインポートできるか。また将来別ツールに移る際のエクスポートも重要です。",
          "モバイル対応: PWAやネイティブアプリの有無。チームメンバーがモバイルでも使うなら必須の観点です。",
        ],
      },
      {
        heading: "候補を見るときの注意点",
        content: [
          "スター数だけで判断しない: GitHubスター数はプロジェクトの注目度を示しますが、実際の安定性や機能の完成度とは異なります。最終コミット日やIssueの対応状況も確認しましょう。",
          "日本語対応の確認: UIの日本語化、ドキュメントの日本語版の有無は、チーム全体での導入ハードルに直結します。",
          "Notionのすべてを置き換えようとしない: Notionは多機能ゆえに、1つのOSSで完全代替するのは難しい場合があります。ドキュメントはOSS-A、タスク管理はOSS-Bのように組み合わせる戦略も有効です。",
        ],
      },
    ],
    checklist: [
      "必要な機能を優先順位付けする（ドキュメント/DB/タスク/Wiki）",
      "セルフホスト可能な環境があるか確認する",
      "チームの技術力に合った導入難易度か判断する",
      "データ移行の手段を事前に調べる",
      "モバイル利用の要否を確認する",
    ],
    relatedLinks: [
      { label: "Notionの代替OSSツール一覧", href: "/alternatives/notion" },
